...
for (const decision of ["Approve", "Deny"] as const) {
  test(`browser ${decision.toLowerCase()} completes a separately running CLI sign-in`, async ({
    sbServer,
    page,
  }) => {
    ...
    try {
      ...
      await page
        .getByRole("button", {
          name: decision === "Deny" ? "Cancel" : decision,
          exact: true,
        })
        .click();
      await expect(page.locator("body")).toContainText("Decision saved");
      expect(await cli.exited, cli.stderr()).toBe(
        decision === "Approve" ? 0 : 1,
      );
      if (decision === "Deny") {
        await expect(access(configPath)).rejects.toThrow();
        expect(cli.stderr()).toContain("Sign-in cancelled in the browser");
        return;
      }
