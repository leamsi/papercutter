...
2. Choose Google Workspace, Pocket ID, or Other OIDC.
...
### Pocket ID and other providers
Create a confidential OIDC client, register the displayed callback and enable Authorization Code with PKCE. Use the provider's issuer URL and client credentials. The provider must supply a stable subject and a verified email, using the `openid`, `email` and `profile` scopes. See [Pocket ID's documentation](https://pocket-id.org/docs/introduction).
